...
MoDx supports HTTPS via mbedTLS. The TLS library is statically linked into libmodx.so, so no extra runtime dependencies are required.
...
MoDx 通过 mbedTLS 支持 HTTPS。TLS 库被静态链接到 libmodx.so 中，因此不需要额外安装运行时依赖。
